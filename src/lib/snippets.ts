...
function toCurlSnippetData(model: RequestSectionState): SnippetData {
    return {
        request: {
            ...
            body: "{'x':'y'}",
            headers: model.headers.filter(x => x.enabled).map(x => ({ name: x.name, value: x.value }))
        }
    }
}
...
function toFetchSnippetData(model: RequestSectionState): SnippetData {
    return {
        request: {
            url: model.url,
            method: model.method.toUpperCase(),
            headers: model.headers ? model.headers.filter(x => x.enabled).map(x => ({ name: x.name, value: x.value })) : [],
            body: "",
            bodyisJson: false,
        }
    }
...
function toSnippetData(model: RequestSectionState, lang: SupportedSnippetLang): SnippetData {
    switch (lang) {
        case "js":
...
export function getCodeSnippet(model: RequestSectionState, lang: SupportedSnippetLang) {
